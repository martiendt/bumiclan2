...
for (const score of scoreStore.$state.scores) {
  data.push({
    name: score.name,
    score: score.contributionMeeting.score
  })
}
  ...
  <div class="main-content-container">
    <div class="main-content-header">
      <div class="flex flex-col gap-4">
        <h1>Contribution Meeting</h1>
        <component
          :is="BaseBreadcrumb"
          :breadcrumbs="[{ name: 'Bumiclan', path: '/' }, { name: 'Contribution Meeting' }]"
        />
      </div>
    </div>
    <div class="main-content-body">
      <div class="card card-template">
        <div class="flex flex-wrap gap-2">
          <button class="btn btn-sm btn-primary">November 2023</button>
        </div>
